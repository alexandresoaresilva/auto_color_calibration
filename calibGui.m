...
%%%%%%%% color calibration image and coordinates and figures and axes %%%%%%%%%%%%%%%%%%
        ...
        donewithcaptureButton           matlab.ui.control.Button
        checker_path
   end
   methods
       function calibGui = calibGui(calibGui)
           ...
           % appName = mfilename;
            ...
            create_color_calib_window_components(calibGui);
            if nargout == 0
                clear calibGui
            end
       end
       ...
       function calibGui = create_color_calib_window_components(calibGui)
            ...
            calibGui.make_selection.Text = {'Select the source (file/ camera) of your '; 'reference Macbeth color checker'};
        ...
        function calibGui = openfileButtonPushed(calibGui, event)
            ...
            if ~isnumeric(calibGui.checker_path)
                ...
                delete_1st_dialog_elements(calibGui);
                creat_norm_buttons(calibGui);
            end
        end
        ...
        function calibGui = creat_norm_buttons(calibGui)
            ...
            toggle_colorcalib_uifig_visibility(calibGui);
        end
        ...
        function calibGui = yesButtonPushed(calibGui, event)
            ...
            if calibGui.capture
                create_capture_elements(calibGui);
            else
                color_calibrate_image(calibGui);
                create_calib_itself_components(calibGui)
            end
        end
        ...
        function calibGui = create_capture_elements(calibGui)
            delete_2nd_dialog_elements(calibGui);
            calibGui.capture = 0;
            % Create ready_txt
            calibGui.ready_txt = uilabel(calibGui.color_calibrate);
            calibGui.ready_txt.HorizontalAlignment = 'center';
            calibGui.ready_txt.FontSize = 16;
            calibGui.ready_txt.FontWeight = 'bold';
            calibGui.ready_txt.FontColor = [1 0 0];
            calibGui.ready_txt.Position = [98 86 135 22];
            calibGui.ready_txt.Text = 'ready to capture!';

            % Create CAPTUREcheckerimageButton
            calibGui.CAPTUREcheckerimageButton = uibutton(calibGui.color_calibrate, 'push');
            
            calibGui.noButton.ButtonPushedFcn = @(CAPTUREcheckerimageButtonPushed,event)calibGui.CAPTUREcheckerimageButtonPushed;
            %calibGui.CAPTUREcheckerimageButton.ButtonPushedFcn = createCallbackFcn(calibGui, @CAPTUREcheckerimageButtonPushed, true);
            
            calibGui.CAPTUREcheckerimageButton.BackgroundColor = [0 0.451 0.7412];
            calibGui.CAPTUREcheckerimageButton.FontName = 'Eras Medium ITC';
            calibGui.CAPTUREcheckerimageButton.FontSize = 16;
            calibGui.CAPTUREcheckerimageButton.FontWeight = 'bold';
            calibGui.CAPTUREcheckerimageButton.FontColor = [0 1 1];
            calibGui.CAPTUREcheckerimageButton.Position = [78 13 175 63];
            calibGui.CAPTUREcheckerimageButton.Text = {'CAPTURE'; 'checker image'};
            toggle_colorcalib_uifig_visibility(calibGui);
        end
        ...
        function calibGui =  CAPTUREcheckerimageButtonPushed(calibGui, event)
            %calibGui.capture = 1;
            color_calibrate_image(calibGui);
            calibGui.capture = 0;
            open_RGB_stream(calibGui);
            %create_calib_itself_components(calibGui);
        end
...
%            cd calibrated;
        ...
        function create_calib_itself_components(calibGui)
            delete_2nd_dialog_elements(calibGui);
