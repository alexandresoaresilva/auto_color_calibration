...
%%  CONSTANTS
path1 = 'E:\code\GitHub\auto_color_calibration\checker_imgs';
addpath(path1);


path1 = [path1, '\colorCalTrouble\fails_to_detect_any_patches'];

filter_n=@(n)1/n^2*ones(n);

addpath(path1);

I = imread('dist_checker_rotated.png');
I = imresize(I,[480 640]);
% I_conv = uint8(convn(I,filter_n(35),'same'));
% I = I - I_conv;
I = uint8(convn(I,filter_n(3),'same'));


% k_R = kmeans(double(I(:,:,1)),24);
% k_G = kmeans(double(I(:,:,2)),24);
% k_B = kmeans(double(I(:,:,3)),24);
